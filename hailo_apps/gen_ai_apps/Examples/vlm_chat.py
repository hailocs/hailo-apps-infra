...
from hailo_apps.hailo_app_python.core.common.defines import VLM_MODEL_NAME_H10, RESOURCES_MODELS_DIR_NAME
import cv2
...
try:
    vdevice = VDevice()
    vlm = VLM(vdevice, get_resource_path(resource_type=RESOURCES_MODELS_DIR_NAME, model=VLM_MODEL_NAME_H10))
    ...
    response = vlm.generate_all(prompt=prompt, frames=[image_rgb], temperature=0.1, seed=42, max_generated_tokens=200)
