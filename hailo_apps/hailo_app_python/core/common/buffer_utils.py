...
def get_caps_from_pad(pad: Gst.Pad):
    ...
    if caps:
        ...
        if structure:
            ...
            hailo_logger.debug(f"Caps extracted - Format: {format}, Width: {width}, Height: {height}")
...
def get_numpy_from_buffer(buffer, format, width, height):
    hailo_logger.debug(f"Converting GstBuffer to numpy - Format: {format}, Width: {width}, Height: {height}")
...
def get_numpy_from_buffer_efficient(buffer, format, width, height):
    hailo_logger.debug(f"Efficient conversion GstBuffer to numpy - Format: {format}, Width: {width}, Height: {height}")
    handler = FORMAT_HANDLERS.get(format)
    if handler is None:
        hailo_logger.error(f"Unsupported format: {format}")
        raise ValueError(f"Unsupported format: {format}")
