...
def handle_dot_env(env_path: Path = None) -> Path:
    ...
    if not env_path.is_file():
        hailo_logger.info(f"Creating new .env file at {env_path}")
...
def _persist_env_vars(env_vars: dict, env_path: Path) -> None:
    hailo_logger.debug(f"Persisting environment variables to {env_path}")
    ...
    with open(env_path, 'w') as f:
        for key, value in env_vars.items():
            if value is not None:
                f.write(f"{key}={value}\n")
    hailo_logger.info(f"✅ Persisted environment variables to {env_path}")
...
def set_environment_vars(config, env_path: Path = None) -> None:
    ...
    hailo_logger.debug(f"Initial config values: {config}")

    # Auto-detection logic
    if host_arch == AUTO_DETECT:
        hailo_logger.info("host_arch is 'auto', detecting...")
        host_arch = detect_host_arch()
    if hailo_arch == AUTO_DETECT:
        hailo_logger.info("hailo_arch is 'auto', detecting...")
        hailo_arch = detect_hailo_arch()
    if hailort_version == AUTO_DETECT:
        hailo_logger.info("hailort_version is 'auto', detecting...")
    ...
    hailo_logger.debug(f"Detected tappas_postproc_dir: {tappas_postproc_dir}")
    if not tappas_postproc_dir:
        hailo_logger.warning("Tappas post-processing directory not found. Using default.")
