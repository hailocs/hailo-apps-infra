...
class user_callbacks_class(app_callback_class):
    def __init__(self):
        hailo_logger.debug("Initializing user_callbacks_class | telegram_enabled=%s", TELEGRAM_ENABLED)
        super().__init__()
        self.frame = None
    ...
    def send_notification(self, name, global_id, confidence, frame):
        ...
        if not self.telegram_enabled or not self.telegram_handler:
            hailo_logger.debug("send_notification skipped | enabled=%s handler=%s",
                               self.telegram_enabled, bool(self.telegram_handler))
            return

        # Check if the notification should be sent
        if self.telegram_handler.should_send_notification(global_id):
            hailo_logger.info("Sending Telegram notification | id=%s name=%s conf=%.3f",
                              global_id, name, confidence)
...
def app_callback(pad, info, user_data):
    ...
    for detection in detections:
        ...
        if label == "face":
            ...
            hailo_logger.info("Frame=%s | face detection | id=%s conf=%.1f",
                              frame_idx, track_id, detection_confidence)
            ...
            if len(classifications) > 0:
                for classification in classifications:
                    if classification.get_label() == 'Unknown':
                        hailo_logger.debug("Frame=%s | id=%s | recognition=Unknown", frame_idx, track_id)
                        string_to_print += 'Unknown person detected'
                    else:
                        hailo_logger.info("Frame=%s | id=%s | recognition=%s conf=%.1f",
                                          frame_idx, track_id, classification.get_label(), classification.get_confidence())
                    ...
                    if track_id > user_data.latest_track_id:
                        hailo_logger.debug("Updating latest_track_id: %s -> %s", user_data.latest_track_id, track_id)
...
def main():  
    ...
    hailo_logger.debug("Options | mode=%s ui=%s", pipeline.options_menu.mode, pipeline.options_menu.ui)
