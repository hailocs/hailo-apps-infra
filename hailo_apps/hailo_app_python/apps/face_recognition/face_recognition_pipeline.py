...
class GStreamerFaceRecognitionApp(GStreamerApp):
    def __init__(self, app_callback, user_data, parser=None):
        ...
        parser.add_argument(
            "--mode", default="run", help="The mode of the application: run, train, delete"
        )
        parser.add_argument(
            "--visualize",
            action="store_true",
            help="In run mode & CLI only, whether display the live visualization of the embeddings",
        )
        parser.add_argument(
            "--ui", action="store_true", help="Whether display the Gradio UI or just CLI"
        )
        super().__init__(parser, user_data)

        self.embedding_queue = (
            multiprocessing.Queue()
        )  # Create a queue for sending embeddings to the visualization process
        ...
        self.json_file = open(
            get_resource_path(
                ...
                model=FACE_ALGO_PARAMS_JSON_NAME,
            ),
            "r+",
        )
        ...
        self.lance_db_vector_search_classificaiton_confidence_threshold = self.algo_params[
        ...
        self.train_images_dir = get_resource_path(
            pipeline_name=None, resource_type=FACE_RECON_DIR_NAME, model=FACE_RECON_TRAIN_DIR_NAME
        )
        ...
        self.hef_path_detection = get_resource_path(
            pipeline_name=FACE_DETECTION_PIPELINE, resource_type=RESOURCES_MODELS_DIR_NAME
        )
        ...
        self.post_process_so_tracker_update = get_resource_path(
            pipeline_name=None,
            resource_type=RESOURCES_SO_DIR_NAME,
            model=TRACKER_UPDATE_POSTPROCESS_SO_FILENAME,
        )
        ...
        if self.options_menu.mode == "run":
            ...
            if self.options_menu.ui:
                self.webrtc_frames_queue = FIFODropQueue(
                    maxsize=2
                )  # smaller sizes are better for live stream
        ...
        self.threads = []
        for _i in range(self.num_worker_threads):
    ...
    def get_pipeline_string(self):
        ...
        )  # 'identity name' - is a GStreamer element that does nothing, but allows to add a probe to it
        ...
        if self.options_menu.mode == "train":
            source_pipeline = (
                f"multifilesrc location={self.current_file} loop=true num-buffers=30 ! "  # each image 30 times
    ...
    def run(self):
        if self.options_menu.mode == "run":
            if (
                self.options_menu.visualize and not self.options_menu.ui
    ...
    def run_training(self):
        ...
        if not os.listdir(self.train_images_dir):
            print(
                f"Training directory {self.train_images_dir} is empty. Copying default training images from local resources."
            )
            source_dir = get_resource_path(
                pipeline_name=None,
                resource_type=DEFAULT_LOCAL_RESOURCES_PATH,
                model=FACE_RECON_LOCAL_SAMPLES_DIR_NAME,
            )
        ...
        for person_name in os.listdir(
        ...
        ):  # Iterate over subfolders in the training directory
            person_folder = os.path.join(self.train_images_dir, person_name)
            if self.db_handler.get_record_by_label(
                label=person_name
            ):  # Ensure the person exists in the database, or create a new record
                continue
            ...
            for image_file in os.listdir(
                ...
                self.current_file = os.path.join(
                    person_folder, image_file
                )  # Set the current file for pipeline processing, used internally in get_pipeline_string
    ...
    def start_visualization_process(self):
        ...
        db_records = (
            self.db_handler.get_all_records()
        )  # Get a copy of the records to avoid shared memory issues
        p = multiprocessing.Process(
            target=self.display_visualization_process, args=(db_records, self.embedding_queue)
        )
    ...
    def display_visualization_process(db_records, embedding_queue):
        ...
        while True:  # Append the new embeddings to the plot
            try:
                embedding_vector, label = embedding_queue.get(
                    timeout=0.1
                )  # Get new embedding from the queue
                visualizer.add_embeddings_to_existing_plot(
                    embeddings=[embedding_vector], labels=[label]
                )
    ...
    def connect_vector_db_callback(self):
        ...
        if identity:
            ...
            )  # trigger - when the pad gets buffer
    ...
    def save_image_file(self, frame, image_path):
        ...
        image.save(
            image_path, format="JPEG", quality=85
        )  # Save as a compressed JPEG with quality 85
        ...
        if hasattr(self, "visualization_process") and self.visualization_process:
            try:
                ...
                self.visualization_process = (
                    None  # Clear the reference to prevent multiple termination attempts
                )
            except Exception as e:
    ...
    def vector_db_callback(self, pad, info, user_data):
        ...
        for detection in (
            ...
            embedding = detection.get_objects_typed(
                hailo.HAILO_MATRIX
            ...
            if len(embedding) > 1:
                print(
                    f"Warning: Multiple embeddings found for track ID {track_id}. Using the first one."
                )
            ...
            person = self.db_handler.search_record(
                embedding=embedding_vector
            )  # most time consuming operation - search the database for the person with the closest embedding
            new_confidence = 1 - person["_distance"]
            classification = detection.get_objects_typed(hailo.HAILO_CLASSIFICATION)
            if classification:
                if (
                    classification[0].get_confidence() < new_confidence
                ):  # ensuring always there is exactly one classification object in the detection - the one with the highest confidence
                    detection.remove_object(classification[0])
                    detection.add_object(
                        hailo.HailoClassification(
                            type="face_recon", label=person["label"], confidence=new_confidence
                        )
                    )
            else:
                detection.add_object(
            ...
            if (
                self.options_menu.visualize and person["label"] != "Unknown"
            ):  # If visualization is active, send the embedding to the visualization process - in case of new uknown person, don't plot - since the Uknown might be become later recognized in better frame after self.skip_frames try
                try:
                    self.embedding_queue.put(
                        (embedding_vector, person["label"]), timeout=0.1
                    )  # Use non-blocking put with a short timeout
                except Exception as e:
                    hailo_logger.warning(
                        f"Failed to add embedding to queue: {e}. Queue might be full or process might not be running."
                    )
    ...
    def train_vector_db_callback(self, pad, info, user_data):
        ...
        for detection in (
            ...
            if (
                len(embedding) != 1
            ):  # we will continue if new embedding exists - might be new person, or another image of existing person
                continue  # if cropper pipeline element decided to pass the detection - it will arrive to this stage of the pipeline without face embedding.
            detection.remove_object(
                embedding[0]
            )  # in case the detection pointer tracker pipeline element (from earlier side of the pipeline) holds is the same as the one we have, remove the embedding, so embedding similarity won't be part of the decision criteria
            ...
            self.add_task(
                "save_image", frame=cropped_frame, image_path=image_path
