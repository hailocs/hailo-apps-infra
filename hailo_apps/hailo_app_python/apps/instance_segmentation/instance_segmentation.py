...
class user_app_callback_class(app_callback_class):
    def __init__(self):
        ...
        hailo_logger.debug("Initialized user_app_callback_class with frame_skip=%d", self.frame_skip)
...
def app_callback(pad, info, user_data):
    ...
    if user_data.get_count() % user_data.frame_skip != 0:
        hailo_logger.debug("Skipping frame %d due to frame_skip=%d", user_data.get_count(), user_data.frame_skip)
        return Gst.PadProbeReturn.OK
    ...
    for detection in detections:
        ...
        hailo_logger.debug("Detection found: label=%s confidence=%.2f bbox=%s", label, confidence, bbox)
            ...
            if user_data.use_frame:
                ...
                if len(masks) != 0:
                    ...
                    if x_max > x_min and y_max > y_min:
                        hailo_logger.debug("Overlaying mask for track_id=%d at (%d,%d) to (%d,%d)", track_id, x_min, y_min, x_max, y_max)
