...
class DatabaseVisualizer:
    ...
    def create_blank_figure(self):
        ...
        ax.text(
            0.5,
            0.5,  # Position the text in the center
    ...
    def add_embeddings_to_existing_plot(
        ...
        if cropped_frames is None:
            cropped_frames = [None] * len(
                embeddings
            )  # Default to None if no cropped frames are provided

        for embedding, label, cropped_frame in zip(
            embeddings, labels, cropped_frames, strict=False
        ):
            # Transform the embedding to 2D using the existing PCA (NumPy-based)
            principal_components, mean = (
                self.global_pca
            )  # Unpack the stored PCA components and mean
            centered_embedding = embedding - mean  # Center the embedding
            reduced_embedding = np.dot(
                centered_embedding, principal_components
    ...
    def visualize(self):
        ...
        reduced_embeddings, principal_components, mean = self.perform_pca(
            all_embeddings, n_components=2
        )

        # Create a mapping from original embeddings to reduced embeddings
        embedding_map = {
            tuple(embedding): reduced
            for embedding, reduced in zip(all_embeddings, reduced_embeddings, strict=False)
        }
        ...
        self.global_pca = (
            ...
            mean,
        )  # Store principal components and mean for later use
        colors = [
            "blue",
            "green",
            "red",
        ...
        for idx, (_record_id, data) in enumerate(record_data.items()):
            ...
            reduced_record_embeddings = np.array(
                [embedding_map[tuple(embedding)] for embedding in data["embeddings"]]
            )
            ...
            ax.scatter(
                reduced_avg_embedding[0],
                reduced_avg_embedding[1],
            ...
            for _embedding, reduced_point, image_path in zip(
                data["embeddings"], reduced_record_embeddings, data["images"], strict=False
            ):
